...
pyiw.c
 ...
 This file is part of pyiw python wireless interface module.

 pyiw is free software: you can redistribute it and/or modify
 it under the terms of the GNU General Public License as published by
 the Free Software Foundation, either version 2 of the License, or
 (at your option) any later version.

 pyiw is distributed in the hope that it will be useful,
 but WITHOUT ANY WARRANTY; without even the implied warranty of
 MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 GNU General Public License for more details.

 You should have received a copy of the GNU General Public License
 along with pyiw.  If not, see <http://www.gnu.org/licenses/>.


 This module was originally developed by Jeremy Moles <jeremy@emperorlinux.com> 
 for Python 2.x. 

 This module was inspired by (and uses) the iw library that comes with the 
 Linux Wireless Tools package. The scanning portions of this module are 
 moderately modified versions of those already found in iwlist.c; however, in 
 the future I plan to clean them up (especially the goto, wtf?)
...
#define PYIW_VERSION_MINOR  4
#define PYIW_VERSION_BUGFIX 0
#define PYIW_VERSION_STRING "0.4.0"
...
typedef pyiw_WirelessInterfaceObject wiface;
...
static int Py_SetValString(PyObject* arg, char* val, int maxsize) {
	...
	if(PyBytes_Check(arg)) {
		strncpy(val, PyBytes_AsString(arg), maxsize);
    ...
    if(PyBytes_Check(arg)) {
		...
		/* WEP */
			...
			if(PyBytes_Check(arg)) {
				...
				len = iw_in_key(
					PyBytes_AsString(val), wi->info.b.key
				);
		...
		/* CHANNEL */
            ...
            if(!Py_SetValDouble(val, &(wi->info.b.freq), 1.0, 12.0)) {
                ...
                printf("iw set ext: %i \n",iw_set_ext(wi->sock, wi->ifname, SIOCSIWFREQ, &wreq));
                if(iw_set_ext(wi->sock, wi->ifname, SIOCSIWFREQ, &wreq)){
                  PyErr_SetString(PyIWError, "Operation not permitted");
                  return -1;
                }
				else return 0;
            }
...
static PyTypeObject pyiw_WirelessInterfaceType = {
    PyVarObject_HEAD_INIT(NULL, 0)
    "pyiw.WirelessInterface",             /* tp_name */
    sizeof(pyiw_WirelessInterfaceObject), /* tp_basicsize */
    0,                         /* tp_itemsize */
...
PyInit_pyiw(void) 
{
    ...
    pyiw_WirelessInterfaceType.tp_new = PyType_GenericNew;
    if (PyType_Ready(&pyiw_WirelessInterfaceType) < 0)
	return NULL;

    m = PyModule_Create(&pyiwmodule);
    if (m == NULL)
	return NULL;
    PyIWError = PyErr_NewException("pyiw.error", NULL, NULL);
    
    Py_INCREF(&pyiw_WirelessInterfaceType);
    Py_INCREF(PyIWError);
    PyModule_AddObject(m, "WirelessInterface", (PyObject *)&pyiw_WirelessInterfaceType);
    PyModule_AddObject(m, "error", PyIWError);
    return m;
}
